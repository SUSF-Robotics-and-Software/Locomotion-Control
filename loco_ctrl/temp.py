...
curv_m_Rb = 0.1 #Input 1/curve radius(1/m)
rov_speed_mss_Lm = -0.01 #Input rover speed(m/s)
...
def params_straight(motor_angle, motor_rot_speed):
    ...
    motor_speed = (rov_speed_mss_Lm / wheel_radius)
    ...
    if isclose(rov_speed_mss_Lm, 0, abs_tol = rov_speed_min):

        status_rpt["stationary"] = 1
        motor_speed = 0

    #Check Rover is reversing
    elif rov_speed_mss_Lm < 0:
...
def motor_rot_speed_calc(motor_rot_speed):
    ...
    motor_speed_left = rov_speed_mss_Lm * abs((curv_radius - rover_width) / (wheel_radius * curv_radius))
    motor_speed_right = rov_speed_mss_Lm * abs((curv_radius + rover_width) / (wheel_radius * curv_radius))
